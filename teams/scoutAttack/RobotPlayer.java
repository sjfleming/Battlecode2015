...
public class RobotPlayer {
	...
	// broadcast channels:
	static int scoutID = minerShuffle + 1;
		...
		if(areWeInScoutedAttack == 1)
		{
			...
			if(rc.getID()==scoutRobotID) // i am the scout
			{
				...
				if(enemyPower > ATTACK_THRESHOLD)
				{
					...
					squadTarget = new MapLocation(scoutEnemyDetectionLocationX,scoutEnemyDetectionLocationY); // for now, since the rallyChan thing isn't working in this code
			...
			else // i am not the scout
			{
				...
				if(attack == 0) // we are retreating
				{
					...
					squadTarget = new MapLocation(x,y); // for now, since the rallyChan thing isn't working in this code
