...
public class RobotPlayer {
	...
	static void doMinerFactory()
	{
		try {
			...
			if (curOre - lastOre < 25 && nmine < numMiners && curOre > 100)
			{
	...
	static void doDrone()
	{
		try {
			updateSquadInfo();
            /*Direction toDest = myLocation.directionTo(squadTarget);
	...
	private static void minerHarass() throws GameActionException {
		if(rc.isCoreReady()){
			...
			if(rc.getHealth()<50 && rc.getFlashCooldown()==0){ // arbitrary danger level of 50 can survive 2 tank hits and a drone shot...
				...
				scaledX = (int) (sqrt[Math.abs(scaledX)]*Math.signum(scaledX));
				scaledY = (int) (sqrt[Math.abs(scaledY)]*Math.signum(scaledY));
